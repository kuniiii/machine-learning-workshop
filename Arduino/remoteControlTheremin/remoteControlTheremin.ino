...
void setup() {
  ...
  // Switch on onboard LED so we know setup is done when running standalone
...
void dispatchAddress(OSCMessage &msg) {
  ...
  if (msg.isFloat(0)) {
    ...
    // Map value to range
    // Output values are roughly the range of human hearing
    // https://en.wikipedia.org/wiki/Audio_frequency
    int mappedVal = map(val, 0, 1 * m, 16, 2048);
    // Call function to set tone
    playTone(mappedVal);
  }
}
